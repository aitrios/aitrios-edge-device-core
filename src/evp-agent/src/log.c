...
static struct elog_entry *
component_to_entry(uint8_t component)
{
	int i;
...
void
SystemDlog(int priority, const char *tag, const char *file, int line,
	   const char *fmt, ...)
{
	va_list list;
	...
	if (UtilityLogWriteDLog(MODULE_ID_SYSTEM, log_level,
				"[%s] %s-%d: %s\n", log_level_str, file, line,
				logstr) != kUtilityLogStatusOk) {
...
int
SystemRegElog(uint8_t component, uint8_t init_value, const char *msg)
{
	int ret = 0;
...
int
SystemSetELog(uint8_t component, uint8_t code)
{
	int ret = 0;
	...
	if (entry) {
		...
		if (entry->code != code) {
			...
			uint16_t event_id =
				(((uint16_t)component << 8) | (uint16_t)code);
			if (UtilityLogWriteELog(MODULE_ID_SYSTEM,
...
uint8_t
SystemGetELog(uint8_t component)
{
	...
	for (i = 0; i < ELOG_ARRAY_SIZE; i++) {
		...
		if (entry->updated) {
			SystemDlog(LOG_DEBUG, "ELOG", __func__, __LINE__,
				   ...
				   "msg:%s \n",
				   entry->component, entry->code,
				   entry->updated ? "true" : "false",
				   entry->msg);
