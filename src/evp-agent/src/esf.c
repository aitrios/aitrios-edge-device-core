...
int evp_agent_esf_init_proxy_cache(void)
{
	...
	if (prm == NULL) {
		fprintf(stderr,
			"failed to allocate memory for EsfNetworkManagerParameter");
		ret = -ENOMEM;
		goto end;
	}
	...
	if (ret != kEsfNetworkManagerResultSuccess) {
		fprintf(stderr, "EsfNetworkManagerLoadParameter failed (%u)",
			ret);
		ret = false;
		goto end;
	}
	...
	if (prm->proxy.url[0] != '\0') {
		...
		ret = snprintf(g_proxy_cache.port, sizeof(g_proxy_cache.port),
			       "%d", prm->proxy.port);
...
static int read_cert_key(enum config_key key, char *buf, size_t buf_size)
{
	...
	if (nread <= 0) {
		fprintf(stderr, "failed to read from %s, nread=%zd errno=%d",
			cert_key_path, nread, errno);
		ret = -EIO;
		goto end;
	}
